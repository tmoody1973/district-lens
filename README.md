...
| Models | Gemini 3.1 Pro for reasoning (`gemini-3.1-pro-preview`); Gemini 3.1 Flash-Lite for extraction + output classifier. Authenticated via Vertex AI ADC with `GOOGLE_CLOUD_LOCATION=global` (3.x preview models are global-endpoint only). | Hackathon mandates Gemini; ADK is Gemini-native; Flash-Lite is cheapest at high volume; ADC removes the need to manage a `GEMINI_API_KEY` secret. |
...
| Storage | MongoDB Atlas project `DistrictLens` with M0 free cluster `districtlens` on GCP `CENTRAL_US` during build, upgraded to M10 for the hackathon demo week then dropped back. Atlas Search + Atlas Vector Search handle all retrieval. | Drops Elastic entirely; same-region as Cloud Run for low-latency reads. |
