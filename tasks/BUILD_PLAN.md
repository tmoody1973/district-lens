...
| A2 | Create dedicated Atlas project `DistrictLens` and M0 cluster `districtlens` on GCP `CENTRAL_US`; create db user `districtlens` (role `atlasAdmin`) with random 20-char password; whitelist current IP; populate `agent/app/.env` `MONGODB_URI` | `mongosh "$MONGODB_URI" --eval 'db.runCommand({ping:1})'` returns `ok: 1`; `db.runCommand({buildInfo:1}).version` reports MongoDB 8.x; cluster region is `CENTRAL_US` per `atlas clusters describe` | §2.6, §2.7, §2.10 | — |
| A3 | Obtain Geocod.io API key on free tier; populate `agent/app/.env` `GEOCODIO_API_KEY` | `curl "https://api.geocod.io/v1.12/geocode?q=...&fields=cd120&api_key=$KEY"` returns 200 with `results[0].fields.congressional_districts[].district_number` populated; verified against `1600 Pennsylvania Avenue NW, DC 20500` returning rooftop accuracy and a single congressional district | §3.5 | — |
| A4 | Configure Gemini auth via Vertex AI ADC (no API key); populate `agent/app/.env` with `GOOGLE_GENAI_USE_VERTEXAI=True`, `GOOGLE_CLOUD_PROJECT=civicsync-440613`, `GOOGLE_CLOUD_LOCATION=global` | `cd agent && uv run python -c "from google import genai; genai.Client().models.generate_content(model='gemini-3.1-pro-preview', contents='ping')"` returns a real response with `usage_metadata.prompt_token_count > 0` | §2.9, §3.3 | A1 |
| A5 | Run `agent/deployment/terraform/single-project/` (App SA, Cloud Run service, GCS log bucket, BigQuery telemetry dataset, log sinks). Then create Artifact Registry Docker repo `districtlens-agent` in `us-central1` and Workload Identity Federation pool `github-actions` + OIDC provider `github` (restricted to `tmoody1973` GitHub org) + CI deployer SA `districtlens-agent-cicd` with deploy roles, all via gcloud | `terraform apply` clean (31 resources added, 0 changed, 0 destroyed); `gcloud artifacts repositories describe districtlens-agent --location=us-central1` succeeds; `gcloud iam workload-identity-pools describe github-actions --location=global` shows ACTIVE; CI SA holds `run.developer`, `artifactregistry.writer`, `iam.serviceAccountUser`, `storage.admin`, `cloudbuild.builds.builder`; authenticated curl to the Cloud Run URL returns HTTP 200 | §2.5, §2.11 | A1 |
| A6 | Configure GitHub repo `tmoody1973/district-lens` vars + secrets so the existing workflows can deploy via WIF. Set repo **Variables**: `REGION=us-central1`, `CICD_PROJECT_ID=civicsync-440613`, `STAGING_PROJECT_ID=civicsync-440613`, `PROD_PROJECT_ID=civicsync-440613`, `GCP_PROJECT_NUMBER=655022470154`, `ARTIFACT_REGISTRY_REPO_NAME=districtlens-agent`, `CONTAINER_NAME=districtlens-agent`, `LOGS_BUCKET_NAME_STAGING=civicsync-440613-districtlens-agent-logs`. Set repo **Secrets**: `WIF_POOL_ID=github-actions`, `WIF_PROVIDER_ID=github`, `GCP_SERVICE_ACCOUNT=districtlens-agent-cicd@civicsync-440613.iam.gserviceaccount.com` | All 8 variables + 3 secrets visible in repo Settings → Secrets and variables → Actions; a workflow_dispatch run of `staging.yaml` (or a push to main) authenticates via WIF and reaches `gcloud run deploy` without a credential error | §2.5, §2.11 | A5 |
| A7 | Populate `agent/app/.env` for local dev with all 7 required env vars (`GOOGLE_GENAI_USE_VERTEXAI`, `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, `MONGODB_URI`, `GEOCODIO_API_KEY`, `ADDRESS_HASH_SALT`, `INTERNAL_API_TOKEN`) | `cd agent && uv run python -c "from app.agent import root_agent"` succeeds; before/after model callbacks (`check_input`, `check_output`) wired; `lookup_district_placeholder` tool present; all 7 env vars resolve via `dotenv` | §2.4 | A2, A3, A4 |
