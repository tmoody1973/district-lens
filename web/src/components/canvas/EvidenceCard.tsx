...
const EVIDENCE_TYPE_LABEL: Record<string, { label: string; color: string }> = {
  direct_quote: { label: "direct quote", color: "text-evidence-direct" },
  questionnaire: { label: "questionnaire", color: "text-evidence-questionnaire" },
  voting_record: { label: "voting record", color: "text-evidence-voting" },
  reported: { label: "reported stance", color: "text-evidence-reported" },
};
...
function evidenceLabel(evidence: EvidenceCardType): { label: string; color: string } {
  ...
  return {
    label: c,
    color:
      c === "direct quote" ? "text-evidence-direct" : c === "paraphrase" ? "text-evidence-reported" : "text-ink-muted",
  };
}
...
export function EvidenceCard({ evidence, compact = false }: Props) {
  ...
  return (
    <div className="rounded-[2px] border-l-4 border-l-purple-500 border border-edge bg-surface-raised p-4 space-y-2">
      <div className="flex items-center gap-2">
        {!compact && (
          <span className="rounded-full bg-purple-900/30 border border-purple-700/40 px-2 py-0.5 text-xs font-semibold text-purple-400 uppercase tracking-wider">
      ...
      <p className="text-sm font-semibold text-ink">{evidence.candidateName}</p>

      <p className="text-sm text-ink-muted leading-relaxed italic whitespace-pre-wrap">
        {evidence.answer}
      </p>
            ...
            <div key={i} className="flex items-start gap-1.5">
              ...
              <div>
                <a
                  ...
                  className="text-xs text-evidence-questionnaire hover:underline"
                >
                  {s.title}
                </a>
                {s.date && <span className="text-xs text-ink-faint ml-1">· {s.date}</span>}
                {s.archived && (
                  <span
                    className="ml-1.5 inline-flex items-center gap-0.5 rounded-[2px] bg-green-900/30 border border-green-700/40 px-1 py-0.5 text-[10px] font-medium text-evidence-direct"
